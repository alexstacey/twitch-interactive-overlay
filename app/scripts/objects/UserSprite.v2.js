...
export default class UserSprite extends BaseSprite {
  ...
  constructor(config) {
    ...
    console.log('created UserSprite for', config.user, config.flags);
  ...
  handleChatEvent({ user, message, flags, method, args, applyAll = false }) {
    ...
    // eslint-disable-next-line no-console
    console.log('handleChatEvent', method, this[method]);
    if (func) {
      if (args) {
        func.call(this, args);
      } else {
  ...
  selectAnimation() {
    ...
    if( this.lurking ) {
      this.setTexture('character_inactive');
    } else {
      this.setTexture('character_active');
    }
  }
